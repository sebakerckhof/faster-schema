...
This package is mainly meant as a limited, but faster alternative to [FasterSchema](https://github.com/aldeed/simple-schema-js/), with a very similar API. [Here's a complete comparison](comparison.md).
In short: the main difference is that this package doesn't support validating against mongoDB modifiers. But in our [tests](https://github.com/sebakerckhof/schema-comparison) on an advanced schema with a large object, it is about ~15x faster for validation and ~30x faster for cleaning of data.
