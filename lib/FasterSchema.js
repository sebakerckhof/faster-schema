...
class FasterSchema {
  ...
  static merge(...schemas) {
    const initial = schemas.shift().clone();
  ...
  extend(otherSchema) {
    if (otherSchema instanceof FasterSchema) {
      ...
      otherSchema = cloneDeep(otherSchema._compiled.definitions);
    }
  ...
  clone() {
    ...
    clone._compiled = cloneDeep(this._compiled);
    return clone;
  }
  ...
  getObjectSchema(key) {
    const parent = `${key}.`;
    const schema = {};
    const definitions = this._compiled.definitions;
...
export const ErrorTypes = FasterSchema.ErrorTypes;
