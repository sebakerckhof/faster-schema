...
class FasterSchema {
  ...
  messageForError(errorInfo) { //eslint-disable-line
    ...
    if (typeof dm.messages[dm.initialLanguage][type] !== 'function') {
      // If the error type is not defined, we return the type name as error
      return type;
    }
  ...
  _addDefinition(name, definition, { checkParent = true, merge = false } = {}) {
    ...
    if (oldDefinition) {
      ...
      definition.required = definition.required || definition.optional === false;
