...
describe('FasterSchema', function () {
  ...
  describe('merge', function () {
    ...
    it('works for empty arrays', function () {
      ...
      expect(() => schema.validate({})).to.not.throw();
    });

    it('works when properties are overwritten', function () {
      let schema = new FasterSchema({ value: { type: String, optional: true } });
      schema = FasterSchema.merge(schema, ({ value: { type: String, optional: false } }));
      expect(() => schema.validate({})).to.throw();

      let schema2 = new FasterSchema({ value: { type: String, optional: true } });
  ...
  it('should print correct instance name of object in the error', function () {
    const scheme = new FasterSchema({
      data: ArrayBuffer,
    });

    expect(() => scheme.validate({ data: {} })).to.throw('Data must be of type ArrayBuffer');
  });

  it('should be able to define custom error types', function() {
    ...
    expect(() => schema.validate({ password: 'foo', confirmPassword: 'bar' })).to.throw('Passwords do not match');
  });

  it('should return the error type name if no message is defined for the error type', function() {
    const schema = new FasterSchema({
      foo: {
        type: String,
        custom() {
          return 'undefinedErrorType';
        },
      },
    });
    expect(() => schema.validate({ foo: 'bar' })).to.throw('undefinedErrorType');
  });
});
