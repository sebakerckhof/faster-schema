...
describe('FasterSchema', function () {
      ...
      expect(context.validationErrors()).to.eql([
        {
          ...
          name: 'foo',
          label: 'Foo',
          type: FasterSchema.ErrorTypes.EXPECTED_TYPE,
          value: 'string',
        },
      ]);
    });
      ...
      expect(context.validationErrors()).to.eql([
        {
          name: 'foo.bar',
          label: 'Bar',
          type: FasterSchema.ErrorTypes.VALUE_NOT_ALLOWED,
          value: 'cold',
        },
      ]);
    });
      ...
      expect(context.validationErrors()).to.eql([
        {
          name: 'a.b',
          label: 'B',
          type: FasterSchema.ErrorTypes.REQUIRED,
        },
      ]);
  ...
  it('null in array is not valid', function () {
    ...
    expectErrorOfTypeLength(FasterSchema.ErrorTypes.EXPECTED_TYPE, schema, {
      names: [null],
    });
  });
  ...
  it('ignore option', function () {
    ...
    expectValid(schema, {
      ...
      ignore: [FasterSchema.ErrorTypes.KEY_NOT_IN_SCHEMA],
    });
    ...
    expectErrorOfTypeLength(FasterSchema.ErrorTypes.KEY_NOT_IN_SCHEMA, schema, {
      bar: 'foo',
    });
  ...
  it('ClientError', function () {
    ...
    function verify(error) {
      ...
      expect(error.details[0].type).to.equal(FasterSchema.ErrorTypes.EXPECTED_TYPE);
      expect(error.details[0].message).to.equal('Int must be of type Integer');
      expect(error.details[1].name).to.equal('string');
      expect(error.details[1].type).to.equal(FasterSchema.ErrorTypes.REQUIRED);
