...
function checkNumber({ def, value }) {
  const expectsInteger = !def.decimal;
  ...
  if (def.max !== null && (!!def.exclusiveMax ? def.max <= value : def.max < value)) {
  ...
  if (def.min !== null && (!!def.exclusiveMin ? def.min >= value : def.min > value)) {
